...
export default function SimilaryProduct({data}) {
    ...
    function SlideItem({ img, title ,id}) {
        return (
            <Stack sx={{ gap: "5px", alignItems: "center", backgroundColor: "white", padding: "10px", width: "200px",height:"250px", cursor: "pointer" }} onClick={() => {
    ...
    return (
        <Box sx={{ margin: "20px 4% 0px 4%", padding: "10px 1%", backgroundColor: "#f2f2eb", display: "flex", flexDirection: "column", gap: "10px" }}>
            <Typography variant='h2' sx={{ color: "text.main", fontSize: "22px" }}>Similar Product</Typography>
            <Slider style={{ overflow: "hidden" }} {...settings}>
                {
                    data?.map((curr, indx) => (
                        <SlideItem key={indx} img={curr.image} title={curr.title} id={curr.id}/>
                    ))
                }
            </Slider>
        </Box>
